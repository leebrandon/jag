...
rtTask = rtGetTasks()
rtTask.setRtUrl(app.config['ARTIFACTORY_URL'])
rtTask.setRtCreds((app.config['RT_USER'], app.config['RT_PASS']))
...
def buildFinder():
    rt = rtLib()
    rt.setRtUrl(app.config['ARTIFACTORY_URL'])
    ...
    if request.method == 'GET':
        try:
            URL = '%s/api/builds' % app.config['ARTIFACTORY_URL']
            jsonData = requests.get(URL, timeout=10)
